...
public class NavitationLayout extends RelativeLayout {
    ...
    public void setNavLine(Activity context, int height, int color, int currentPosition) {
        if (textViews != null) {
            navWidth = (int) getScreenWidth(context) / textViews.length;

            //navWidth = (int) ((getScreenWidth(context) / textViews.length)-getResources().getDimension(R.dimen.dp_30));
        }
        XLog.e("--------width--------" + navWidth);
        height = dip2px(context, height);
    ...
    public void setViewPager(final Context context, String[] titles, ViewPager viewPager, final int unselectedcolor,
                             final int setectedcolor, int txtUnselectedSize, final int txtSelectedSize, final int currentPosition,
                             int widOffset, boolean smoothScroll, int splilinecolor, float splilinewidth, float topoffset, float bottomoffset) {
        this.viewPager = viewPager;
        this.txtUnselectedColor = unselectedcolor;
        this.txtSelectedColor = setectedcolor;
        this.txtUnselectedSize = txtUnselectedSize;
    ...
    private void moveBar(View bar, int width, float percent, int position) {
        ...
        //  lp.width = textViews[position].getWidth();
    ...
    public void setTxtSelectedColor(int position, String selectColor,String unSelectColor) {
        for (int i = 0; i < textViews.length; i++) {
            if (i == position) {
                textViews[i].setTextColor(Color.parseColor(selectColor));
            }else {
                textViews[i].setTextColor(Color.parseColor(unSelectColor));
            }
