...
public class TagAliasOperatorHelper {
                ...
                case DELAY_SEND_ACTION:
                    if(msg.obj !=null && msg.obj instanceof  TagAliasBean){
                        ...
                        if(context!=null) {
                         ...
                         XLog.w(TAG,"#unexcepted - msg obj was incorrect");
                    }
                    break;
                case DELAY_SET_MOBILE_NUMBER_ACTION:
                    if(msg.obj !=null && msg.obj instanceof  String) {
                         XLog.i(TAG, "retry set mobile number");
                        sequence++;
    ...
    public void handleAction(Context context,int sequence, TagAliasBean tagAliasBean){
        ...
        if(tagAliasBean == null){
             XLog.w(TAG,"tagAliasBean was null");
            return;
        }
        ...
        if(tagAliasBean.isAliasAction){
            switch (tagAliasBean.action){
                ...
                case ACTION_SET:
                    ...
                    break;
                default:
                     XLog.w(TAG,"unsupport alias action type");
                    return;
            }
        }else {
            switch (tagAliasBean.action) {
                ...
                case ACTION_CLEAN:
                    ...
                    break;
                default:
                     XLog.w(TAG,"unsupport tag action type");
                    return;
            }
        }
    }
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
         XLog.i(TAG,"action - onTagOperatorResult, sequence:"+sequence+",tags:"+jPushMessage.getTags());
         XLog.i(TAG,"tags size:"+jPushMessage.getTags().size());
        init(context);
        ...
        if(jPushMessage.getErrorCode() == 0){
             XLog.i(TAG,"action - modify tag Success,sequence:"+sequence);
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage){
        int sequence = jPushMessage.getSequence();
         XLog.i(TAG,"action - onCheckTagOperatorResult, sequence:"+sequence+",checktag:"+jPushMessage.getCheckTag());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean)setActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
             XLog.i(TAG,"tagBean:"+tagAliasBean);
            setActionCache.remove(sequence);
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
         XLog.i(TAG,"action - onAliasOperatorResult, sequence:"+sequence+",alias:"+jPushMessage.getAlias());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean)setActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
             XLog.i(TAG,"action - modify alias Success,sequence:"+sequence);
            setActionCache.remove(sequence);
    ...
    public void onMobileNumberOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
         XLog.i(TAG,"action - onMobileNumberOperatorResult, sequence:"+sequence+",mobileNumber:"+jPushMessage.getMobileNumber());
        init(context);
        if(jPushMessage.getErrorCode() == 0){
             XLog.i(TAG,"action - set mobile number Success,sequence:"+sequence);
            setActionCache.remove(sequence);
        }else{
