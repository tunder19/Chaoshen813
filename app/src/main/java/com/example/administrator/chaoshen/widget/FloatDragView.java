...
public class FloatDragView {
    ...
    private ImageView getFloatDragView(View.OnClickListener clickListener) {
        if (mImageView != null) {
            ...
            mImageView.setImageResource(R.drawable.price_select);
    ...
    private void setImageViewNearEdge(final View v) {
        ...
        if (v.getLeft() < ((SystemUtil.getScreenSize(context).x) / 2)) {
            // 设置位移动画 向左移动控件位置
            ...
            final TranslateAnimation animation = new TranslateAnimation(0, (SystemUtil.getScreenSize(context).x
            ...
            animation.setAnimationListener(new Animation.AnimationListener() {
                ...
                public void onAnimationEnd(Animation arg0) {
                    ...
                    RelativeLayout.LayoutParams lpFeedback = new RelativeLayout.LayoutParams(
                            RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
                    lpFeedback.setMargins(SystemUtil.getScreenSize(context).x - v.getWidth(), v.getTop(), 0, 0);
                    v.setLayoutParams(lpFeedback);
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                        v.postInvalidateOnAnimation();
                    }
                    lastPosition[0] =SystemUtil.getScreenSize(context).x - v.getWidth();
                    lastPosition[1] = v.getTop();
                }
            });
            v.startAnimation(animation);
    ...
    private void setScreenHW(Activity context) {
        if (mScreenHeight < 0) {
            ...
            Point screen = SystemUtil.getScreenSize(context);
            mScreenWidth = screen.x;
            mScreenHeight = screen.y - SystemUtil.getStatusBarHeight(context);
        }
    }
