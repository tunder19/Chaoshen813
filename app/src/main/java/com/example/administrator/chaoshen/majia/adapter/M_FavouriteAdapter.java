...
public class M_FavouriteAdapter extends AppBaseAdapter {
    private M_MatchListBean mdata;
    private int layout = R.layout.m_layout_score_child_item;

    public M_FavouriteAdapter(Context context, M_MatchListBean data) {
        super(context);
        this.mdata = data;
    }
    ...
    @Override
    public int getCount() {
        return mdata.getMatchListBeanList().size();
    }

    @Override
    public Object getItem(int position) {
        return mdata.getMatchListBeanList().get(position);
    }
    ...
    public View getView(int position, View converview, ViewGroup parent) {
        ...
        if (converview == null) {
            ...
            converview = View.inflate(mContext, R.layout.m_layout_score_child_item, null);
      ...
      /*  if (position == matches.size() - 1) {
        ...
        MatchListBean data = mdata.getMatchListBeanList().get(position);
    ...
    public void setData(M_MatchListBean data) {
        mdata = data;
    ...
    public int[] getArrayId() {
        List<MatchListBean> list = mdata.getMatchListBeanList();
    ...
    public void notifyCache(M_MatchListBean newCache) {
       /* M_MatchListBean oldCache = getList();
        List<String >  deleteMatchID=new ArrayList<>();
        for (int i = 0; i < oldCache.getMatchListBeanList().size(); i++) {
            if (oldCache.getMatchListBeanList().get(i).isSaveLocal()==false){
                deleteMatchID.add(oldCache.getMatchListBeanList().get(i).getMatchId()+"");
            }
        }

        List<MatchListBean> lastList=new ArrayList<>();
        //将新的清除
        for (int i = 0; i <newCache.getMatchListBeanList().size() ; i++) {
            for (int j = 0; j <deleteMatchID.size() ; j++) {
                if (!deleteMatchID.get(j).equals(newCache.getMatchListBeanList().get(i).getMatchId()+"")){
                    lastList.add(newCache.getMatchListBeanList().get(i));
                }
            }
        }
        M_MatchListBean newData=new M_MatchListBean();
        newData.setMatchListBeanList(lastList);*/
        XCache xCache = XCache.get(mContext);
        xCache.put(IntentKey.SAVE_MATCHES, newCache);
